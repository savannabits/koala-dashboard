...
class ArticleController  extends Controller
{
    ...
    public function dt(Request $request) {
        return DataTables::of(Article::query())
            ->addColumn("actions",function($model) {
                $actions = '';
                if (\Auth::user()->can('articles.show')) $actions .= '<button class="btn bg-primary mr-2" title="View Details" data-action="show-article" data-tag="button" data-id="'.$model->id.'"><i class="fas fa-eye"></i></button>';
                if (\Auth::user()->can('articles.edit')) $actions .= '<button class="btn bg-warning mr-2" title="Edit Item" data-action="edit-article" data-tag="button" data-id="'.$model->id.'"><i class="fas fa-edit"></i></button>';
                if (\Auth::user()->can('articles.delete')) $actions .= '<button class="btn bg-danger mr-2" title="Delete Item" data-action="delete-article" data-tag="button" data-id="'.$model->id.'"><i class="fas fa-trash"></i></button>';
                return $actions;
            })
            ->rawColumns(['actions'])
            ->make();
    ...
    {
        try {
            $request->authorize('delete',$article);
            $article->delete();
